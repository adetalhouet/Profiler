...
cpu::~cpu() {
	...
	this->clearDataSet(
			(std::vector<google::protobuf::Message*>&) _prevCPUStatus);

	// clean up _currentCPUStatus
	this->clearDataSet(
			(std::vector<google::protobuf::Message*>&) _currentCPUStatus);
}
...
bool cpu::fillCPUInfo(cpuInfo& curCPUInfo, int curCPUNum) {
	...
	if (curCPUNum == -1) {
		// get total CPU usage
		flag = fscanf(statFile, curPattern, &usertime, &nicetime, &systemtime,
		...
		while (fgets(buffer, PATTERNSIZE, statFile)) {
			flag = sscanf(buffer, curPattern, &usertime, &nicetime, &systemtime,
					&idletime, &iowaittime, &irqtime, &softirqtime);
...
void cpu::calcuateCPUUtil(cpuInfo& prevCPUInfo, cpuInfo& curCPUInfo) {
	...
	unsigned long totalIdleTime = curCPUInfo.idletime()
			- prevCPUInfo.idletime();
	unsigned long totalIoWaitTime = curCPUInfo.iowaittime()
			- prevCPUInfo.iowaittime();

	curCPUInfo.set_cputime(totalDeltaTime);

	if (totalIoWaitTime != 0)
		curCPUInfo.set_ioutilization(
				(float) totalIoWaitTime * 100 / totalDeltaTime);

	if (totalIdleTime != 0)
		curCPUInfo.set_cpuutilization(
				100 - ((float) totalIdleTime * 100 / totalDeltaTime));
	else
...
void cpu::processOfflineCPUs() {
	// lookup CPUs
	bool findCPU = false;
	std::vector<cpuInfo*>::iterator iterPrevCPUInfo =
			this->_prevCPUStatus.begin();
	while (iterPrevCPUInfo != this->_prevCPUStatus.end()) {
		findCPU = false;
		std::vector<cpuInfo*>::iterator iterCurCPUInfo =
				this->_currentCPUStatus.begin();
		while (iterCurCPUInfo != this->_currentCPUStatus.end()) {
			if ((*iterPrevCPUInfo)->cpunumber()
					== (*iterCurCPUInfo)->cpunumber()) {
				findCPU = true;
				break;
			}
		...
		if (findCPU == true && (*iterCurCPUInfo)->offline()) {
			...
			(*iterCurCPUInfo)->set_softirqtime(
					(*iterPrevCPUInfo)->softirqtime());
...
void cpu::refresh() {
	...
	// prepare data
	this->moveDataSet(
			(std::vector<google::protobuf::Message*>&) this->_currentCPUStatus,
			(std::vector<google::protobuf::Message*>&) this->_prevCPUStatus);
	...
	while (iterPrevCPUInfo != this->_prevCPUStatus.end()) {
		...
		while (iterCurCPUInfo != this->_currentCPUStatus.end()) {
			...
			if ((*iterCurCPUInfo)->cpunumber()
					== (*iterPrevCPUInfo)->cpunumber()) {
				if ((*iterCurCPUInfo)->offline() == false)
					this->calcuateCPUUtil(*(*iterPrevCPUInfo),
							*(*iterCurCPUInfo));
