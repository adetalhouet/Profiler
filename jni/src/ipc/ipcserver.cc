...
bool ipcserver::init(char* socketName) {
	...
	strcpy(this->serverAddr.sun_path + 1, socketName); // socketName = "profiling_v1"
...
bool ipcserver::bind() {
	...
	int option = true;
	if (::setsockopt(this->serverFD, SOL_SOCKET, SO_REUSEADDR, (char *) &option,
			sizeof(option)) < 0) {
		::close(this->serverFD);
		return (false);
	}

	if (::bind(this->serverFD, (const sockaddr*) &this->serverAddr,
			this->serverLen) < 0) {
		::close(this->serverFD);
		return (false);
	}
...
ipcserver::EVENT ipcserver::poll() {
	...
	// wait for connection for 1 seconds (on blocking mode)
	int result = 0;
//	if (this->hasClient())
